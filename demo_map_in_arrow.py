...
from arrow_array_utils import is_null, structured_list_array_adapter, uniform_arrow_array_adapter
...
def calculate(
    ...
    res = np.empty(id_a.shape, np.float64)
    magnitudes_per_entity = len(magnitude_a) // len(id_a)
    for i in range(id_a.shape[0]):
        ...
        scale = index_a[i] + np.sqrt(magnitude_a[i]) / 121.0
        res[i] = area * scale
    return res
...
def map_in_arrow_func(iterator):
    for batch in iterator:
        ...
        id_bitmap, id_data = uniform_arrow_array_adapter(id_)
        ...
        # print(f"res = {res}")
        yield pa.RecordBatch.from_pydict({"res": pa.array(res)})


if __name__ == "__main__":
    df = join_data()
    df_out = df.mapInArrow(map_in_arrow_func, StructType([StructField("res", DoubleType())]))
    df_out.show()
